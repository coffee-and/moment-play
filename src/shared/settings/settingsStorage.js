...
export const SETTINGS_STORAGE_KEY = "eunContents.settings.preferences";
...
export function getLocalSettings() {
  const data = readRetainedData(SETTINGS_STORAGE_KEY, { maxAgeMs: LOCAL_MAX_AGE_MS });
...
export function saveLocalSettings(partialSettings) {
  ...
  writeRetainedData(SETTINGS_STORAGE_KEY, next);
  return next;
}
