...
export function HomePage() {
  ...
  return (
    <div className={cx("wrap home-page")}>
      <section className={cx("page-content greet reveal d1")} id="top">
        <h1 className={cx("page-title")}>What will you play today?</h1>
        <p>짧은 순간에도 바로 시작할 수 있는 퍼즐과 미니게임을 만나보세요.</p>
      </section>

      <section className={cx("page-content section featured-section")} aria-labelledby="featured-title">
        <button type="button" className={cx("featured reveal d2")} onClick={() => openGame("omok")}>
          <span className={cx("f-body")}>
            <span className={cx("f-tag")}>Featured</span>
            <h2 id="featured-title">OMOK</h2>
            <p>조용히 생각하고, 결정적인 한 수를 놓아보세요.</p>
            <span className={cx("f-action")}>PLAY NOW</span>
          </span>
          <FeaturedCatPattern />
        </button>
      </section>

      <section className={cx("page-content section home-games")} aria-label="추천 게임">
        <div className={cx("home-games-grid")}>
          {homeGames.map((game) => (
