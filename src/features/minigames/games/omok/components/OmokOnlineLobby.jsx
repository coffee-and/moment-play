...
export function OmokOnlineLobby({
  ...
  return (
    <div className={cx("omok-game__lobby omok-game__waiting-room")} aria-label="오목 온라인 대기실">
      <div>
        <div className={cx("kicker")}>Online room · {roomCode}</div>
        <h3 className={cx("omok-game__section-title")}>{room.title}</h3>
        <p className={cx("omok-game__hint")}>Host는 흑, Guest는 백입니다.</p>
      </div>
      {errorMessage ? <p className={cx("omok-game__notice is-error")} role="alert">{errorMessage}</p> : null}
      {syncWarning ? <p className={cx("omok-game__notice")} role="status">{syncWarning}</p> : null}
      <div className={cx("omok-game__invite")}>
        <label className={cx("f-label")} htmlFor="omok-invite-url">Invite URL</label>
        <div className={cx("omok-game__invite-row")}>
          <input className={cx("txt")} id="omok-invite-url" type="text" value={inviteUrl} readOnly />
          <Button type="button" variant="secondary" onClick={onCopyInvite} disabled={!inviteUrl}>
            {copied ? "복사됨" : "복사"}
          </Button>
        </div>
        <span className={cx("visually-hidden")} role="status">{copied ? "초대 링크가 복사되었습니다." : ""}</span>
      </div>
      <div className={cx("omok-game__room-slots")}>
        {playersByRole.map(([role, player]) => (
          <div className={cx("omok-game__room-slot")} key={role}>
            <span className={cx(`omok-game__dot ${role === ONLINE_PLAYER_ROLE.HOST ? "is-black" : "is-white"}`)} aria-hidden="true" />
            <div>
              <div className={cx("omok-game__player-name")}>
                {player ? `${player.nickname}${player.userId === currentUserId ? " · me" : ""}` : "Waiting"}
              </div>
              <div className={cx("omok-game__player-status")}>
      ...
      <div className={cx("omok-game__rule-panel")} aria-label="공유 방 규칙">
        <div className={cx("omok-game__rule-panel-header")}>
          <span className={cx("omok-game__player-guide-role")}>공유 방 규칙</span>
          <button
            type="button"
        ...
        <p className={cx("omok-game__hint")}>{getRoomRuleSummaryText(room)}</p>
        {isHost ? (
          <>
            <div className={cx("omok-game__settings")} role="group" aria-label="오목 규칙">
              {GAME_MODE_OPTIONS.map((mode) => (
                <button
                  className={cx(`omok-game__setting-chip${room.gameMode === mode ? " is-selected" : ""}`)}
                  type="button"
                  disabled={busy}
                  key={mode}
      ...
      <div className={cx("omok-game__rule-panel")} aria-label="플레이어 금수 안내 설정">
        <span className={cx("omok-game__player-guide-role")}>플레이어 금수 안내</span>
        {playersByRole.map(([role, player]) => {
          const isMe = Boolean(player && player.userId === currentUserId);
          return (
            <div className={cx("omok-game__player-guide-row")} key={role}>
              <span className={cx("omok-game__player-guide-role")}>{ONLINE_ROLE_LABEL[role]}</span>
              {!player ? (
                <p className={cx("omok-game__hint")}>입장 대기 중</p>
              ) : isMe ? (
                room.gameMode === OMOK_MODE.STANDARD ? (
                  <div className={cx("omok-game__settings omok-game__settings--toggles")}>
                ...
                <p className={cx("omok-game__hint")}>{getPlayerGuideSummaryText(player)}</p>
              )}
            </div>
          );
        })}
      </div>
      <div className={cx("game-stage__actions omok-game__waiting-actions")}>
