...
export function MemoryGameOverlays({
  ...
  return (
    ...
    >
        ...
        >
          ...
          {phase === MEMORY_PHASE.IDLE ? (
            <>
              ...
              <Button className={cx("memory-game__primary")} type="button" onClick={startGame} disabled={rankingSubmission.isStarting}>
                {rankingSubmission.isStarting ? "랭킹 게임 준비 중…" : "게임 시작"}
              </Button>
            </>
          ) : (
            <>
              <p className={cx("memory-game__state-kicker")} aria-label={`현재 ${round}라운드`}>
                — {round} ROUND —
              </p>
              <p className={cx("memory-game__state-title memory-game__state-title--countdown")}>
      ...
      {isExitConfirmOpen ? (
        <GameStageModal
          className={cx("memory-game__state-view")}
          role="dialog"
          aria-modal="true"
          aria-labelledby="memory-game-exit-title"
        >
          <h3 className={cx("memory-game__state-title")} id="memory-game-exit-title">게임을 나갈까요?</h3>
          <p>현재 라운드 진행은 저장되지 않아요.</p>
          <div className={cx("memory-game__state-actions game-stage-modal__actions")}>
      ...
      {phase === MEMORY_PHASE.TURN_READY && !isExitConfirmOpen ? (
        <GameStageModal
          className={cx("memory-game__transition-view memory-game__transition-view--turn")}
          data-state="turn-ready"
          role="status"
          aria-live="polite"
        >
          <p className={cx("memory-game__transition-title")}>YOUR TURN</p>
          <p className={cx("memory-game__transition-copy")}>순서대로 선택하세요</p>
        </GameStageModal>
      ) : null}

      {phase === MEMORY_PHASE.CLEARED && !isExitConfirmOpen ? (
        <GameStageModal
          className={cx("memory-game__transition-view memory-game__transition-view--clear")}
          data-state="cleared"
          role="status"
          aria-live="assertive"
        >
          <p className={cx("memory-game__transition-title")}>ROUND {round} CLEAR!</p>
          {combo >= 2 ? <p className={cx("memory-game__transition-copy")}>{combo} COMBO</p> : null}
        </GameStageModal>
      ) : null}

      {phase === MEMORY_PHASE.REPLAYING && !isExitConfirmOpen ? (
        <GameStageModal className={cx("memory-game__transition-view memory-game__transition-view--replay")} role="status" aria-live="assertive">
          <p className={cx("memory-game__transition-title")}>REPLAY!</p>
          <p className={cx("memory-game__transition-copy")}>같은 라운드를 한 번 더 보여드릴게요.</p>
        </GameStageModal>
      ) : null}
      ...
      {phase === MEMORY_PHASE.COMPLETED && !isExitConfirmOpen ? (
        ...
        >
          ...
          <h3 className={cx("memory-game__state-title")}>10 ROUND CLEAR!</h3>
      ...
      {phase === MEMORY_PHASE.PAUSED && !isExitConfirmOpen ? (
        ...
        >
          <h3 className={cx("memory-game__state-title")} id="memory-game-pause-title">일시정지</h3>
          <div className={cx("memory-game__state-actions game-stage-modal__actions")}>
            <Button className={cx("memory-game__state-button")} type="button" onClick={resumeGame}>
              계속하기
            </Button>
            <Button className={cx("memory-game__state-button")} variant="secondary" type="button" onClick={resetToIdle}>
              처음부터 다시 시작
            </Button>
            <Button className={cx("memory-game__state-button")} variant="secondary" type="button" onClick={requestExit}>
              게임 나가기
            </Button>
          </div>
        </GameStageModal>
      ...
      {phase === MEMORY_PHASE.FAILED && !isExitConfirmOpen ? (
        ...
        >
          ...
          <h3 className={cx("memory-game__state-title memory-game__state-title--failed")} id="memory-game-result-title">
            {resultTitle}
          </h3>
          <div className={cx("memory-game__state-details")}>
            <p>{round}라운드 실패</p>
          ...
          <div className={cx("memory-game__state-actions game-stage-modal__actions")}>
            {failureStatus !== "over" ? (
              <Button className={cx("memory-game__state-button")} type="button" onClick={retryRound}>
                남은 목숨으로 재도전
              </Button>
            ) : null}
            <Button className={cx("memory-game__state-button")} variant="secondary" type="button" onClick={resetToIdle}>
