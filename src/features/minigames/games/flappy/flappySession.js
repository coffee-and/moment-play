export const FLAPPY_SESSION_CONFIG = Object.freeze({
  courseRoundCount: 5,
...
export function createFlappyEndlessSession() {
  return {
    ...
    round: FLAPPY_SESSION_CONFIG.courseRoundCount,
    roundElapsedMs: 0,
    totalElapsedMs: 0,
  };
}
...
export function advanceFlappySession(session, deltaMs) {
  ...
  if (nextRoundElapsedMs < FLAPPY_SESSION_CONFIG.roundDurationMs) {
    return {
      event: null,
      session: {
        ...session,
  ...
  if (session.round >= FLAPPY_SESSION_CONFIG.courseRoundCount) {
    return {
      event: "course-complete",
      session: {
        ...session,
        roundElapsedMs: FLAPPY_SESSION_CONFIG.roundDurationMs,
        totalElapsedMs: FLAPPY_SESSION_CONFIG.courseRoundCount
          * FLAPPY_SESSION_CONFIG.roundDurationMs,
      },
    };
  }
  ...
  return {
    ...
    session: {
      ...
      roundElapsedMs: nextRoundElapsedMs - FLAPPY_SESSION_CONFIG.roundDurationMs,
      totalElapsedMs: nextTotalElapsedMs,
    },
  };
}
...
export function getFlappyTimeRemainingMs(session) {
  ...
  return Math.max(0, FLAPPY_SESSION_CONFIG.roundDurationMs - session.roundElapsedMs);
}
