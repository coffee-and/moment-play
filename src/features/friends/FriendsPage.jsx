...
function getFriendlyErrorMessage(error, fallback = "요청을 처리하지 못했습니다. 잠시 후 다시 시도해 주세요.") {
  ...
  if (message.includes("Permanent account required")) return "친구 기능은 계정을 만든 뒤 사용할 수 있어요.";
...
export function FriendsPage() {
    ...
    return (
      <section className="wrap friends-page">
        <StatusPanel
          ...
          description="친구 코드를 검색하고 요청을 주고받으려면 로그인해 주세요."
  ...
  if (authStatus === "anonymous") {
    return (
      <section className="wrap friends-page">
        <StatusPanel
          title="계정을 만들면 친구 기능이 열려요"
          description="현재 플레이 기록은 유지하면서 이메일 계정으로 전환할 수 있습니다."
          action={<Button as={Link} to={SIGNUP_PATH}>{AUTH_LABELS.createAccount}</Button>}
        />
      </section>
    );
  }
