...
export function SettingsPage() {
  ...
  return (
    <section className={cx("wrap", "settings-page")} aria-labelledby="settings-title">
      <header className={cx("settings-page__header")}>
      ...
      <div className={cx("settings-grid")}>
        <section className={cx("card", "settings-card", "settings-card--appearance")} aria-labelledby="appearance-title">
          <div className={cx("settings-card__heading")}>
            <div>
          ...
          <div className={cx("theme-choice")} role="group" aria-label="화면 테마 선택">
            <button
              className={cx("theme-choice__option", theme === THEME.LIGHT && "is-selected")}
              type="button"
              aria-pressed={theme === THEME.LIGHT}
              onClick={() => setTheme(THEME.LIGHT)}
            >
              <span className={cx("theme-choice__preview", "theme-choice__preview--light")} aria-hidden="true">
                <span />
                <span />
                <span />
              </span>
              <span className={cx("theme-choice__label")}><strong>라이트</strong><span>밝고 선명하게</span></span>
            </button>
            <button
              className={cx("theme-choice__option", theme === THEME.DARK && "is-selected")}
              type="button"
              aria-pressed={theme === THEME.DARK}
              onClick={() => setTheme(THEME.DARK)}
            >
              <span className={cx("theme-choice__preview", "theme-choice__preview--dark")} aria-hidden="true">
                <span />
                <span />
                <span />
              </span>
              <span className={cx("theme-choice__label")}><strong>다크</strong><span>차분하고 편안하게</span></span>
            </button>
          </div>
        </section>

        <section className={cx("card", "settings-card", "settings-card--account")} aria-labelledby="account-title">
          <div className={cx("settings-card__heading")}>
            <div>
          ...
          {authStatus === "loading" ? (
            <p className={cx("settings-muted")} role="status">계정 정보를 확인하고 있어요.</p>
          ) : isPermanentAccount ? (
            <div className={cx("account-settings")}>
              <div className={cx("account-settings__profile")}>
                <div className={cx("account-settings__identity")}>
                  <span className={cx("settings-avatar")} aria-hidden="true">{accountNickname.slice(0, 1).toUpperCase()}</span>
                  <div>
                ...
                <form className={cx("nickname-settings")} onSubmit={handleNicknameSave}>
                  <label className="f-label" htmlFor="account-nickname">닉네임</label>
                  <div className={cx("nickname-settings__row")}>
                    <input
                      className={cx("txt", "nickname-settings__input")}
                      id="account-nickname"
                      type="text"
                      minLength={2}
                      maxLength={12}
                    ...
                    />
                    <Button
                      className={cx("nickname-settings__save")}
                      size="small"
                      type="submit"
                      disabled={friendStatus !== FRIEND_STATUS.READY || isSavingNickname}
                    >
                  ...
                  <p className={cx("nickname-settings__help")}>친구와 온라인 오목에서 공통으로 사용하는 이름입니다.</p>
                  {nicknameMessage ? (
                    <p
                      className={cx("settings-notice", nicknameMessageType === "error" && "is-error")}
              ...
              <div className={cx("account-settings__meta")}>
                <dl className={cx("account-details")}>
                  <div>
                    <dt>친구 코드</dt>
                    <dd>
                ...
                <div className={cx("settings-actions", "settings-actions--account")}>
            ...
            <div className={cx("account-settings", "account-settings--guest")}>
              <p>로그인하면 랭킹 기록 저장, 친구 관리, 친구 오목 초대를 사용할 수 있어요.</p>
              <Button as={Link} size="small" to={LOGIN_PATH}>{AUTH_LABELS.login}</Button>
            </div>
          )}

          {accountMessage ? <p className={cx("settings-notice", "is-error")} role="alert">{accountMessage}</p> : null}
        </section>

        <section className={cx("card", "settings-card", "settings-card--data")} aria-labelledby="data-title">
          <div className={cx("settings-card__heading")}>
            <div>
          ...
          <div className={cx("data-settings")}>
            <div className={cx("data-settings__copy")}>
            ...
            {!isResetConfirmOpen ? (
              ...
              <div className={cx("reset-confirm")} role="group" aria-label="게임 기록 초기화 확인">
                <p>기기 내 기록을 삭제할까요?</p>
          ...
          {resetMessage ? <p className={cx("settings-notice")} role="status">{resetMessage}</p> : null}
        </section>

        <section className={cx("card", "settings-card", "settings-card--about")} aria-labelledby="about-title">
          <div className={cx("settings-card__heading")}>
            <div>
            ...
            <span className={cx("settings-card__badge")}>v{packageInfo.version}</span>
          </div>

          <div className={cx("about-settings")}>
